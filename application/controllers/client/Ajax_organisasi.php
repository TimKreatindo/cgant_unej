...
class Ajax_organisasi extends CI_Controller {
    public function validation(){
        ...
        switch($act){
            case 'add':
                ...
                if($tipe_bukti == 'file'){
                    $upload_file = $this->app->upload_files($_FILES['bukti'], 'organisasi', './assets/upload/organisasi/');
                    ...
                    if(count($error_upload) > 0){
                        if(!empty($success_upload)){
                            foreach($success_upload as $su){
                                unlink('./assets/upload/organisasi/' . $su['file_name']);
                            }
                        }
                ...
                $data_insert = [
                        'id_user' => $user->id,
                        'organisasi' => $input_post['organisasi'],
                ...
                $this->app->input_data('sh_organisasi', $data_insert);
                break;
            case 'get-edit':
                $id = $input_post['id'];
                $get_data = $this->client->get_organisasi($id)->row();
                if($get_data){
                    $decode_bukti = json_decode($get_data->bukti);

                    $data = [
                        'id' => $get_data->id_encode,
                        'organisasi' => $get_data->organisasi,
            ...
            case 'edit':
                ...
                if(!empty($get_data)){
                    ...
                    $this->db->where('sha1(id)', $id)->update('sh_organisasi', $data_update);
            ...
            case 'delete':
                $id = $input_post['id'];
                $this->app->delete_data('sh_organisasi', 'sha1(id)', $id);
                break;
        }
    }
}