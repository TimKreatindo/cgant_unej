...
class Ajax_rekognisi extends CI_Controller {
    public function validation(){
        ...
        switch($act){
            case 'add':
                ...
                if($tipe_bukti == 'file'){
                    $upload_file = $this->app->upload_files($_FILES['bukti'], 'rekognisi', './assets/upload/rekognisi/');
                    ...
                    if(count($error_upload) > 0){
                        if(!empty($success_upload)){
                            foreach($success_upload as $su){
                                unlink('./assets/upload/rekognisi/' . $su['file_name']);
                            }
                        }
                ...
                $data_insert = [
                        ...
                        'jenis_rekognisi' => $input_post['jenis_rekognisi'],
                        'jenis_kegiatan' => $input_post['jenis_kegiatan'],
                        'level' => $input_post['tingkat_rekognisi'],
                ...
                $this->app->input_data('sh_rekognisi', $data_insert);
                break;
            case 'get-edit':
                $id = $input_post['id'];
                $get_data = $this->client->get_rekognisi($id)->row();
                if($get_data){
                    ...
                    $data = [
                        ...
                        'jenis_rekognisi' => $get_data->jenis_rekognisi,
            ...
            case 'edit':
                ...
                if(!empty($get_data)){
                    ...
                    $this->db->where('sha1(id)', $id)->update('sh_rekognisi', $data_update);
            ...
            case 'delete':
                $id = $input_post['id'];
                $this->app->delete_data('sh_rekognisi', 'sha1(id)', $id);
                break;
        }
    }
}